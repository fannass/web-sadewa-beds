                ...
                <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <select name="room_type" id="room_type" required
                                ...
                                @foreach(\App\Models\RoomType::where('is_active', true)->orderBy('sort_order')->get() as $type)
                                    <option value="{{ $type->name }}" {{ old('room_type', $room->room_type) == $type->name ? 'selected' : '' }}>{{ $type->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="status" id="status" required
                                ...
                                <option value="terisi" {{ old('status', $room->status) === 'terisi' ? 'selected' : '' }}>
                                    Terisi</option>
                                <option value="dibersihkan" {{ old('status', $room->status) === 'dibersihkan' ? 'selected' : '' }}>Dibersihkan</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="bed_count" id="bed_count"
                                value="{{ old('bed_count', $room->bed_count) }}" min="1" max="10"
                                class="w-full px-4 py-3 text-sm rounded-xl border border-gray-200 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 outline-none">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="facilities" id="facilities"
                            value="{{ old('facilities', $room->facilities) }}"
                            class="w-full px-4 py-3 text-sm rounded-xl border border-gray-200 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 outline-none"
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="3"
                            class="w-full px-4 py-3 text-sm rounded-xl border border-gray-200 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 outline-none resize-none">{{ old('notes', $room->notes) }}</textarea>
                    </div>
                </div>
