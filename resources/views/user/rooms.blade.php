                ...
                <div class="stat-card stat-available p-3 sm:p-5 flex items-center gap-3 sm:gap-4">
                    ...
                    <div>
                        <p class="text-[20px] sm:text-[28px] font-bold text-[#0CA15C] leading-none">
                            {{ \App\Models\Room::where('status', 'tersedia')->count() }}
                        </p>
                ...
                <div class="stat-card stat-cleaning p-3 sm:p-5 flex items-center gap-3 sm:gap-4">
                    ...
                    <div>
                        <p class="text-[20px] sm:text-[28px] font-bold text-amber-500 leading-none">
                            {{ \App\Models\Room::where('status', 'dibersihkan')->count() }}
                        </p>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            @forelse($rooms as $room)
                <div class="room-card {{ $room->status === 'tersedia' ? 'available' : '' }} p-5 sm:p-6 group relative">
                    <a href="{{ route('rooms.detail', $room->id) }}" class="block">
                    ...
                    @if($room->status === 'tersedia' && ($whatsapp['whatsapp_enabled'] ?? false))
                        <div class="mt-4 pt-4 border-t border-gray-100">
                            <a href="{{ $whatsapp['whatsapp_url'] }}&text={{ rawurlencode('Halo, saya ingin menanyakan ketersediaan ' . $room->name) }}"
                                target="_blank" rel="noopener noreferrer"
                                class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 text-[12px] sm:text-[13px] font-medium text-[#25D366] bg-[#25D366]/10 hover:bg-[#25D366]/20 border border-[#25D366]/30 rounded-lg transition-all"
